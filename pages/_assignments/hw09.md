...
> * Within the views/posts.py file: please make sure that when you convert the Post objects to JSON, you pass the current_user into the to_dict() method. Example:<br><br>**`json.dumps(post.to_dict(user=self.current_user))`**<br><br>This is necessary for the like / bookmark functionality will work for the React client.
